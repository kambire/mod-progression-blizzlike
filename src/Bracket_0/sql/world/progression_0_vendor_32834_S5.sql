/* ---------------------------------------------------------
   NPC 32834 - BORRAR TODO y CARGAR VENDOR (desde wotlkdb HTML)
   AzerothCore WotLK - MySQL 8.4

   ENFASIS:
   - Este script RESPALDA y BORRA TODO lo existente del vendor
     (npc_vendor y game_event_npc_vendor) para el NPC 32834,
...
DROP TABLE IF EXISTS backupKambi_npc_vendor_32834_20260101;
CREATE TABLE backupKambi_npc_vendor_32834_20260101 LIKE npc_vendor;

INSERT INTO backupKambi_npc_vendor_32834_20260101
SELECT *
FROM npc_vendor
WHERE entry = @npc;
...
DROP TABLE IF EXISTS backupKambi_game_event_npc_vendor_32834_20260101;
CREATE TABLE backupKambi_game_event_npc_vendor_32834_20260101 LIKE game_event_npc_vendor;

INSERT INTO backupKambi_game_event_npc_vendor_32834_20260101
SELECT gev.*
...
DROP TEMPORARY TABLE IF EXISTS tmp_items_32834;
CREATE TEMPORARY TABLE tmp_items_32834 (
...
INSERT INTO tmp_items_32834 (item, arena, honor, rating) VALUES
    (40778,0,60000,0),
    (40779,0,60000,0),
    (40780,0,60000,0),
    (40797,0,50000,0),
...
SELECT COUNT(*) AS tmp_items_rows FROM tmp_items_32834;

-- =========================================================
-- D) RESOLVER ExtendedCost existente (deduplicado por costo)
-- =========================================================

DROP TEMPORARY TABLE IF EXISTS tmp_cost_defs_32834;
CREATE TEMPORARY TABLE tmp_cost_defs_32834 (
...
INSERT INTO tmp_cost_defs_32834 (arena, honor, rating)
SELECT DISTINCT arena, honor, rating
FROM tmp_items_32834
ORDER BY arena, honor, rating;

SELECT COUNT(*) AS distinct_cost_defs FROM tmp_cost_defs_32834;

UPDATE tmp_cost_defs_32834 d
...
SET @missing_cost_defs := (
    SELECT COUNT(*)
    FROM tmp_cost_defs_32834
    WHERE extendedCostId IS NULL
);

SELECT @missing_cost_defs AS missing_cost_defs;
SELECT arena, honor, rating FROM tmp_cost_defs_32834 WHERE extendedCostId IS NULL ORDER BY arena, honor, rating;
SELECT IF(@missing_cost_defs = 0, 'OK - ExtendedCost resueltos', 1/0) AS extendedcost_status;

-- =========================================================
-- E) INSERT: vendor en npc_vendor (entry=32834)
-- =========================================================
...
SELECT @npc AS entry,
       ...
       0 AS VerifiedBuild
FROM tmp_items_32834 i
JOIN tmp_cost_defs_32834 d
  ON d.arena = i.arena
 AND d.honor = i.honor
...
SELECT COUNT(*) AS npc_vendor_items_32834
FROM npc_vendor
WHERE entry = @npc;
