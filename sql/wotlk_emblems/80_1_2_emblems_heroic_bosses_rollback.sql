...
-- If @HAS_BACKUP = 0, nothing to rollback.
SELECT @BRACKET AS bracket, @HAS_BACKUP AS backup_table_present;

-- Use a temp table of keys to safely delete current modified rows.
...
-- Delete current emblem rows on those keys (only emblem items)
DELETE cl
...
-- Re-insert original rows from backup (avoid duplicates)
SET @SQL := CONCAT(
...
-- GM reload note:
--   .reload creature_loot_template (si existe) o restart worldserver.
